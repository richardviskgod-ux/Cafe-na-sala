...
export default function Clients() {
  ...
  const [formData, setFormData] = useState({ name: "", cpf: "", phone: "", birthday: "" });
  ...
  const createClient = useCreateClient({
    mutation: {
      onSuccess: () => {
        ...
        setIsAddOpen(false);
        setFormData({ name: "", cpf: "", phone: "", birthday: "" });
    ...
    <DashboardLayout>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1,2,3,4].map(i => <Card key={i} className="animate-pulse h-64 bg-white/5" />)}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredClients?.map(client => (
            <Card key={client.id} className="group hover:border-primary/30 transition-all duration-300">
              ...
              <CardContent className="pt-4">
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div className={client.balance > 0 ? "bg-destructive/10 rounded-xl p-3 border border-destructive/20" : "bg-emerald-500/10 rounded-xl p-3 border border-emerald-500/20"}>
                    <p className="text-xs text-muted-foreground mb-1">Saldo Devedor</p>
                    <p className={`font-bold ${client.balance > 0 ? 'text-destructive' : 'text-emerald-400'}`}>
                ...
                <div className="flex gap-2">
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      if(confirm('Excluir cliente? O histórico será perdido.')) {
        ...
        <DialogContent onClose={() => setIsAddOpen(false)}>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input required value={maskCPF(formData.cpf)} onChange={e => setFormData({...formData, cpf: e.target.value})} maxLength={14} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">Telefone</label>
              <Input required value={maskPhone(formData.phone)} onChange={e => setFormData({...formData, phone: e.target.value})} maxLength={15} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/80">Data de Aniversário (Opcional)</label>
              <Input type="date" value={formData.birthday} onChange={e => setFormData({...formData, birthday: e.target.value})} className="[&::-webkit-calendar-picker-indicator]:filter-[invert(1)]" />
            </div>
      ...
      {/* Transaction Modal */}
      <Dialog open={transactionType !== null} onOpenChange={(open) => !open && closeTransactionModal()}>
        <DialogContent onClose={closeTransactionModal}>
          <DialogHeader>
            <DialogTitle className={transactionType === 'purchase' ? 'text-destructive' : 'text-emerald-400'}>
              {transactionType === 'purchase' ? 'Registrar Nova Compra (Fiado)' : 'Registrar Pagamento'}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleTransaction} className="space-y-4 mt-4">
            ...
            <Button 
              type="submit" 
              className={`w-full mt-6 h-14 text-lg ${transactionType === 'purchase' ? 'bg-destructive hover:bg-destructive/90' : 'bg-emerald-500 hover:bg-emerald-600 text-white'}`}
