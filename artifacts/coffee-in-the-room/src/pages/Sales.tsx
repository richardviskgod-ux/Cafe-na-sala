...
export default function Sales() {
  const [formData, setFormData] = useState({ clientName: "", productName: "", paymentMethod: "Dinheiro" });
  ...
  const createSale = useCreateSale({
    mutation: {
      onSuccess: () => {
        ...
        setFormData({ clientName: "", productName: "", paymentMethod: "Dinheiro" });
  ...
  const getPaymentIcon = (method: string) => {
    switch(method) {
      case 'Pix': return <QrCode className="w-4 h-4 text-emerald-400" />;
      case 'Cartão': return <CreditCard className="w-4 h-4 text-blue-400" />;
      default: return <Banknote className="w-4 h-4 text-amber-400" />;
    }
  };
    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <CardContent className="p-6">
              <form onSubmit={handleCreate} className="space-y-5">
                <div className="space-y-2">
                  ...
                  >
                    <option value="" disabled className="bg-[#2b0a3d]">Selecione um cliente (ou Avulso)</option>
                    <option value="Avulso" className="bg-[#2b0a3d] font-bold">Cliente Avulso</option>
                    {clients?.map(c => (
                      <option key={c.id} value={c.name} className="bg-[#2b0a3d]">{c.name} - {c.cpf}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    <option value="" disabled className="bg-[#2b0a3d]">Selecione um produto</option>
                    {products?.filter(p => p.stock > 0).map(p => (
                      <option key={p.id} value={p.name} className="bg-[#2b0a3d]">{p.name} (R$ {p.price.toFixed(2)})</option>
                    ))}
                  </select>
                </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-xl font-display font-bold text-white mb-4 px-2">Histórico Recente</h3>
          
          {loadingSales ? (
            <div className="space-y-3">
              {[1,2,3,4].map(i => <Card key={i} className="animate-pulse h-20 bg-white/5" />)}
            </div>
          ) : (
            <div className="space-y-3">
              {sales?.slice().reverse().map(sale => (
                <Card key={sale.id} className="bg-black/20 border-white/5 hover:bg-black/40 transition-colors">
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex flex-col">
                      <span className="font-bold text-lg text-white">{sale.productName}</span>
